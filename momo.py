...
# meta description: Поиск сообщений по ключевым словам во всех доступных чатах с детальной информацией.
# meta license: MIT
...
class SearchMessagesByKeywordsMod(loader.Module):
    """Модуль для поиска сообщений по ключевым словам во всех доступных чатах."""
        ...
        async for dialog in self._client.iter_dialogs():
            ...
            try:
                async for msg in self._client.iter_messages(chat_id, search=keywords, from_user=target_user):
                    ...
                    results.append(
                        f"📌 Чат: {utils.escape_html(chat_name)}\n"
                        f"🔗 [Сообщение]({msg_link})\n"
                        f"🕒 Дата: {msg_time}\n"
                        f"👤 Автор: {msg_author}\n"
                    )
            except Exception as e:
                # Игнорируем чаты с ошибками доступа
                continue
